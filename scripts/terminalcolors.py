...
Description
    This script provides functionality for color testing in a terminal.
...
Development
    ...
    module-esque namespaces; i.e. having classes with static methods that
    never get instatiated. Hence, it makes calls at the module level
    and is not really importable, limiting reuse.
...
options.foreground = options.foreground and (options.numbers or options.hex
                                             or options.rgb)
...
class Term16(object):
    ...
    def _render():
        ...
        if options.foreground:
            ...
            def esc(n):
                return bg_escape % n + fg_escape % (15 if n < 9 else 0)
    ...
    def ansicodes():
        ...
        """
        fg_codes = ['0m', '1m'] + [
            fg for pair in [["{}m".format(i), "1;{}m".format(i)]
                            for i in range(30, 38)] for fg in pair
        ]
...
class term256(Term16):
    ...
    def _rgb_color_table():
        ...
        if options.foreground:
            ...
            def render(n): return fg_escape % n + label_num % n + \
    ...
    def _render(palette):
        ...
        if options.foreground:
            ...
            def render(n):
                return term256.label(n, esc) + clear
        else:

            def esc(n): return fg_escape % \
                (16 if term256._to_rgb(n).is_light() else 255)

            def render(n):
                return bg_escape % n + term256.label(n, esc) + clear
...
class term88(Term16):
    ...
    def _rgb_color_table():
        ...
        if options.foreground:
            ...
            def render(n):
                return fg_escape % n + label_num % n + \
                    label_val % str(rgbl[n]) + clear
        else:
    ...
    def _render(palette):
        ...
        if options.foreground:
            ...
            def render(n):
                return term88.label(n, esc) + clear
        else:
            ...
            def render(n):
                return bg_escape % n + term88.label(n, esc) + clear
    ...
    def display():
        ...
        if options.rgb:
            ...
            for row in greys:
                print(padding.join(i for i in row))
...
def _cube_display(colors):
    ...
    if options.horizontal:
            ...
            while colors:
                ...
                for n in range(size):
                    print(
                        padding.join(
                            i for i in
                            rows[n] + tuple(reversed(rows[n + size]))))
                    if options.padding:
                        print(padding)
                if colors:
                    print()
        ...
        def _vertical(colors):
            ...
            bottom = [
                colors[n + size:len(colors):size * 2]
